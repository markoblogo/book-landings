...
export const metadata = metadataBase({
  ...
  description: "Professional books for physical commodity brokers, traders, farmers, and market operators."
});

const faqItems = [
  ["Are the books free?", "Most AMI Team editions on this site are free to open, download, or listen to in the formats shown on their pages. External commercial titles are explicitly marked as Amazon editions."],
...
function AvailabilityBadge({ status, distribution = "free", hero = false }: { status: CatalogBook["status"]; distribution?: CatalogBook["distribution"]; hero?: boolean }) {
...
function BookFeature({ book, reverse }: { book: CatalogBook; reverse: boolean }) {
  ...
  const amazon = book.distribution === "amazon";
  ...
  return (
    <article className={`${styles.bookFeature} ${reverse ? styles.reverse : ""}`} id={book.slug} lang={book.contentLanguage}>
      <div className={styles.printFrame}>
        ...
        <AvailabilityBadge status={book.status} distribution={book.distribution} />
      </div>
        ...
        {available ? (
          <div className={styles.actions} aria-label={`${book.title} formats`}>
            {book.assets.map((asset) => (
              <a key={asset.href} href={asset.href} target="_blank" rel="noopener noreferrer" className={`${styles.download} ${amazon ? styles.downloadAmazon : ""}`}>
                {asset.label}
              </a>
            ))}
          </div>
...
export default function Page() {
  ...
  const fanBooks = englishBooks.filter((book) => book.distribution !== "amazon");
    ...
    <main className={styles.page}>
      <section className={styles.hero} id="top">
        <div className={`${styles.container} ${styles.heroInner}`}>
          <div className={styles.heroCopy}>
            <AvailabilityBadge status="available" distribution="free" hero />
            <h1>Books for physical commodity markets</h1>
            <p>Professional editions for brokers, traders, farmers, and market operators. Built around physical markets, execution, logistics, and commercial work.</p>
          </div>
      ...
      <footer className={styles.footer}>
        <div className={`${styles.container} ${styles.footerGrid}`}>
          <div><p className={styles.footerBrand}>AMI books</p><p>Professional publishing for physical commodity markets.</p></div>
