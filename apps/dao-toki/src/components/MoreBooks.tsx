...
// Stoic-style More Books block: keep the same design/content principle, but only 2 cards.
...
export default function MoreBooks({ dict }: { dict: any }) {
  const isTp = dict?.lang === 'tp';
  const bookLabel = isTp ? dict?.moreBooks?.label ?? 'lipu' : 'BOOK';
    ...
    <section id="more-books" className={styles.section}>
      <div className="container">
        ...
        <div className={styles.grid}>
          {BOOKS.map((b) => {
            const localized = isTp ? dict?.moreBooks?.books?.[b.id] : undefined;
            const title = localized?.title ?? b.title;
            const subtitle = localized?.subtitle ?? b.subtitle;
            const description = localized?.description ?? b.description;
            const alt = isTp ? `sitelen pi ${title}` : `Promo for ${title}`;

            return (
              <article key={b.id} className={`${styles.card} ux-hover-card`}>
                <div className={styles.inner}>
                  <div className={styles.media}>
                    <Image src={b.promoSrc} alt={alt} fill className={styles.promoImg} sizes="(max-width: 900px) 100vw, 50vw" />
                  </div>

                  <div className={styles.content}>
                    <div className={styles.kicker}>{bookLabel}</div>
                    <h3 className={styles.bookTitle}>{title}</h3>
                    <div className={styles.bookSubtitle}>{subtitle}</div>
