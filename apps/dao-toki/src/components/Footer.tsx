        ...
        <div className={styles.grid}>
          <div className={styles.leftCol}>
            <div className={styles.contactRow}>
              ...
              <a href={mailto} data-sitelen-layer-ignore className={`${styles.mailLink} ux-hover-btn ux-focus-ring`}>
                {emailDisplay}
              </a>
              ...
              <div className={styles.socials}>
                ...
                <a href={socialGitHub} target="_blank" rel="noopener noreferrer" className={`${styles.socialLink} ux-hover-btn ux-focus-ring`}>
                  {dict?.contact?.social_github ?? 'GitHub'}
                </a>
              </div>
            </div>
          ...
          <div className={styles.rightCol}>
            <div className={styles.banners}>
              <Banner
                ...
                subtitle={dict?.footer?.banners?.translator?.subtitle ?? 'Translate into toki pona (Latin / sitelen pona / emoji).'}
              />
              <Banner
                variant="learn"
                href="https://stoic.abvx.xyz/"
                title={dict?.footer?.banners?.stoic?.title ?? 'Stoic Wisdom in Toki Pona'}
                subtitle={dict?.footer?.banners?.stoic?.subtitle ?? 'Read Stoic classics in toki pona with the same landing format.'}
                iconText=">"
              />
              <Banner
                variant="kit"
                href="https://toki-free.abvx.xyz/"
                title={dict?.footer?.banners?.kit?.title ?? 'The Toki Pona Reader’s Kit'}
                subtitle={dict?.footer?.banners?.kit?.subtitle ?? 'Free PDF: fast start into reading toki pona (includes Pythagoras full text).'}
