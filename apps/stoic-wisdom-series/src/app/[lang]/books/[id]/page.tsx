...
export async function generateMetadata({ params }: { params: Promise<{ lang: string; id: string }> }) {
  ...
  const imageUrl = openGraphImage(stoicSiteConfig, book.promoImage || book.coverImage) ?? canonicalUrl(stoicSiteConfig, book.promoImage || book.coverImage);

  return {
    title,
    description,
    alternates: {
      canonical: canonicalUrl(stoicSiteConfig, `/${safeLang}/books/${book.id}`),
      languages: {
        en: canonicalUrl(stoicSiteConfig, `/en/books/${book.id}`),
        ['tok' as any]: canonicalUrl(stoicSiteConfig, `/tp/books/${book.id}`),
      } as any,
    },
    openGraph: {
      title,
      description,
      url: canonicalUrl(stoicSiteConfig, `/${safeLang}/books/${book.id}`),
      type: 'book',
      images: [
        {
          url: imageUrl,
