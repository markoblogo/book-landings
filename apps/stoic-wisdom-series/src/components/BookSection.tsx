...
export default function BookSection({ book, dict, isLast }: BookSectionProps) {
    ...
    const giftBadgeLabel = lang === 'tp' ? dict?.hero?.free_badge ?? 'mani ala' : 'FREE';
    const promoAlt = lang === 'tp' ? `sitelen pi ${title}` : `Promo for ${title}`;
            ...
            <div className={`container ${styles.container}`}>
                ...
                <div className={styles.contentCol}>
                    <header className={styles.header}>
                        {isGift && <GiftBadge label={giftBadgeLabel} className={styles.sectionBadge} />}
